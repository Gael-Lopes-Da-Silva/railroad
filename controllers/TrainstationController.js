...
export async function updateTrainstation(request) {
  ...
  return await TrainstationModel.findByIdAndUpdate(request.params.id, {
    ...
    open_hour: request.body.open_hour
      ...
      : trainstation.open_hour,
    close_hour: request.body.close_hour
      ? request.body.close_hour
      : trainstation.close_hour,
    image: request.body.image ? request.body.image : trainstation.image,
  });
}

export async function deleteTrainstation(request) {
  const startStationTrains = await TrainModel.find({
    start_station: request.params.id,
  }); // we get all the trains that start with this trainstation
  const endStationTrains = await TrainModel.find({
    end_station: request.params.id,
  }); // we get all the trains that end with this trainstation
